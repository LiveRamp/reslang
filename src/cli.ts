...
function handle(args: IArguments, version: string, rules: IRules, allFiles: string[], silent: boolean) {
    try {
...
export default function(args: IArguments, version: string) {
    ...
    if (testwrite) {
        ...
        files.forEach(file => {
            ...
            const out = handle(args, version, rules,[fname], true)
            const ftestname = lpath.join(fname, testwrite)
            process.stdout.write(file + " ")
            fs.writeFileSync(ftestname, out + "\n", { encoding: "utf8" })
        })
    } else {
        handle(args, version, rules, files, false)
    }
}
